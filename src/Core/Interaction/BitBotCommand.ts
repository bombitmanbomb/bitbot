...
export class BitBotCommand {
	public permissions: Dictionary<string, List<PermissionString>> =
		new Dictionary();
	public permissionValidationScript!: (
		interaction: Interaction,
		cache: any
	) => IValidation;
	private permissionValidation: Dictionary<string, (com: Interaction) => any> =
		new Dictionary();
	private script!: (
		com: Interaction,
		reply: ReplyHelper,
		validation: IValidation & { pass: boolean }
	) => any;
	private subScripts: Dictionary<string, BitBotCommand["script"]> =
		new Dictionary();
	...
	public setPermission(
		key: string,
		perms: PermissionString | PermissionString[] | List<PermissionString>
	): void {
		if (typeof perms === "string")
			this.permissions.AddOrUpdate(key, List.ToList([perms]), (_, list) => {
				list.Add(perms);
				return list;
			});
		else
			this.permissions.AddOrUpdate(key, List.ToList(perms), (_, list) => {
				list.AddRange(List.ToList(perms));
				return list;
			});
	}

	public async runPermissionValidation(
		interaction: Interaction,
		cache: any
	): Promise<IValidation> {
	...
	public async run(
		...
		if (interaction.isCommand()) {
			...
			if (subCommand) {
				...
				if (
					this.subScripts.TryGetValue(subCommand, script) &&
					script.Out != null
				)
					return void script.Out.call(this, interaction, helper, validation);
			} else
				return void this.script.call(this, interaction, helper, validation);
		}
	 ...
	 */
	public setPermissionCheck(
		valid: BitBotCommand["permissionValidationScript"]
	): void {
...
export type IBitBotCommand = Command &
	(CommandPermission | CommandNoPermission);
