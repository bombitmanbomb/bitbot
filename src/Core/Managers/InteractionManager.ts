...
export class InteractionManager {
    ...
    if (interactionData.isCommand()) {
      ...
      interaction.deferReply({ ephemeral: true }).then(() => {
        ...
        if (this.command.TryGetValue(interaction.commandName, cmd)) {
          ...
          let command = cmd.Out;
          command.runPermissionValidation(interaction, cache).then(async (validation) => {
            //? Is there validation
            if (!(validation.channels.length === 0)) {
              ...
              for (let channelObject of validation.channels) {
                ...
                let cachePerms = await ((this.Bot.Discord.Client.guilds.cache.get(interaction.guildId!)?.channels.cache.get(channelObject.channel_id) as TextChannel)?.fetch())
                  .then(channel => channel.permissionsFor(this.Bot.Discord.Client.user?.id as string, false)) as Readonly<Permissions>;
              ...
              if (failed.length > 0) {
                ...
                for (let perms of failed) {
                  ...
                  for (let perm of perms.permissions) {
                    ...
                    section.Append(`**__${perm.perm}__**: ${perm.meets ? ":white_check_mark:" : ":x:"}\r\n`)
                  }
            ...
            helper.quickReply("Command Not Handled", "Please contact the Bot Owner") // Command Mapped to Discord API but not internally,
          })
        }
      })
   ...
   */
  private quickRespond(interactionData: Interaction, title: string = "Something went Wrong", description?: string) {
  ...
  public registerModule(command: BitBotCommand) {
    switch (command.type) {
      ...
      default:
        throw new BBError.Error("MODULE_TYPE_INVALID", command.type, command.commandId);
    }
  }
  ...
  public async cloudUpdate() {
    let commands = []
    for (let obj of this.command)
      commands.push(obj.Value.command)
